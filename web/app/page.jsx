...
const DISCLAIMER = 'Crash-style rounds come from a provably-fair random number generator: every round is independent of the ones before it, so nothing here can predict the next multiplier. Treat this as a computer-vision and modelling exercise on your own data, not a betting system.';
  ...
  return (
    <main>
      <header className="top">
        <div>
          <h1>Aviator Predictor <span>/ gemini vision + random forest</span></h1>
          <div className="row">
        ...
        <div>
          ...
          <div className="card">
            ...
            <ol style={{ paddingLeft: 18, fontSize: 13.5, lineHeight: 1.7, color: 'var(--muted)', margin: 0 }}>
              <li>The screenshot is sent to Gemini with a JSON schema, so it returns structured round history instead of prose.</li>
              <li>Those rounds become a 31-value feature vector: lags, rolling moments, streaks, rates, time-of-day.</li>
              <li>A random forest predicts the next multiplier in log space; two classifier heads predict P(&ge;2x) and P(&ge;10x).</li>
              <li>Everything is scored against a naive base-rate baseline, so you can see whether the model found anything.</li>
            </ol>
          </div>
        </div>
        <div>
